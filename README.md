...
- **AI 反推中文提示词**：任意网页图片悬停右上角浮标 / 右键菜单 → 一键反推可复用的中文提示词，过程动画 + 复制结果。
...
1. **页面悬停浮标**：把鼠标移到任意网页图片上，图片右上角出现「AI 反推中文提示词」按钮，点击即可。
2. **右键菜单**：图片上点击右键 → 「AI 反推中文提示词」。
3. **手动 URL**：在侧边栏首页粘贴图片 URL，点击「手动反推这张图」。

触发后侧边栏自动打开 → 显示图片预览与四段过程动画（读取图片 → 编码 → AI 视觉识别 → 整理提示词）→ 完成后展示可复制的中文提示词。

> 提示：请在「设置 → AI 大模型」选用**支持视觉**的模型（如 `gpt-4o`、`doubao-1-5-vision-pro`、`qwen-vl-*`、`moonshot-v1-*-vision-preview`、`glm-4v-*` 等）。
